...
def load_data():
    fullcorpus = pd.read_csv("data/tweets.csv")
...
def load_preprocessed():
    preprocessed = pd.read_csv(os.path.join(dirname, 'data/preprocessed_data.csv'), sep='\t')
    return preprocessed


def load_data_for_training():
    dataset = pd.read_csv('data/data_for_training.csv', sep='\t')
...
def load_xtest():
    result = pd.read_csv('data/X_test.csv', sep='\t')
    return result


def load_xtest_rt():
    result = pd.read_csv('data/X_test_rt.csv', sep='\t')
    return result


def load_xtest_fv():
    result = pd.read_csv('data/X_test_fv.csv', sep='\t')
    return result


def load_keywords():
    result = pd.read_csv('data/keywords.csv', sep='\t')
    return result


def load_positive():
    positive_words = open(os.path.join(dirname, 'dicts/emotions_dictionary/positive.txt'), "r")
    positive_words = positive_words.read().splitlines()
    return positive_words


def load_negative():
    negative_words = open(os.path.join(dirname, 'dicts/emotions_dictionary/negative.txt'), "r")
    negative_words = negative_words.read().splitlines()
    return negative_words


def data_collected():
    collected_data = pd.read_csv('data/data_collected.csv', header=0)
